...
const HiddenUsers = () => {
  ...
  const [openPopup, setOpenPopup] = useState(router.query.openPopup === "true" || false);
          ...
          {D_blockedUsers.map(({ nickname, id, listed }) => (
            <li key={id}>
              <div className={styles.userProfile}>
                <Image src={DefaultProfImg.src} alt={"프로필 이미지"} width={32} height={32} />
                <span>{nickname}</span>
                <Gold />
              </div>
      ...
      {openPopup && (
        <>
          <ConfirmPopup
            ...
            content={
              <>
                <span className={styles.boldText}>이 사용자의 게시물을 다시 보시겠어요?</span>
                <span>
